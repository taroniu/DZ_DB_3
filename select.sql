...
SELECT count(song)
 ...
 WHERE album_id = albums.id
   AND date BETWEEN '01-01-2019' AND '31-12-2020';

--средняя продолжительность треков по каждому альбому
SELECT avg(duration), album
  FROM songs, albums a
 WHERE album_id = a.id
 GROUP BY a.album;
...
SELECT DISTINCT(collection_name)
  FROM collection c, songs_collection sc, songs s, albums a, artists_albums aa, artists aaa
 WHERE c.id  = sc.collection_id
   AND sc.song_id = s.id
   AND s.album_id = a.id
   AND a.id = aa.albums_id
   AND aa.artists_id = aaa.id
   AND aaa.artist  = 'slipknot';
...
SELECT DISTINCT(album)
  FROM albums, artists_albums, artists a, artists_janres
 WHERE (SELECT count(janres_id)
  FROM artists_janres) > 1;
